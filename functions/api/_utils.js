...
export async function cols(env, table) { const r = await env.DB.prepare(`PRAGMA table_info(${table})`).all(); return (r.results || []).map(x => x.name); }
export async function ensureAll(env) {
  ...
  await env.DB.prepare(`CREATE TABLE IF NOT EXISTS tournament_draws (id INTEGER PRIMARY KEY AUTOINCREMENT, tournament_id INTEGER NOT NULL, status TEXT DEFAULT 'DRAFT', groups_json TEXT NOT NULL, draw_code TEXT, created_at TEXT DEFAULT CURRENT_TIMESTAMP, updated_at TEXT DEFAULT CURRENT_TIMESTAMP, published_at TEXT)`).run();
}
export async function openTournament(env) { await ensureAll(env); return await env.DB.prepare(`SELECT t.*, e.name AS event_name, e.code AS event_code FROM tournaments t LEFT JOIN event_types e ON e.id=t.event_type_id WHERE t.status IN ('OPEN','LOCKED','DRAW_DRAFT','DRAW_PUBLISHED') ORDER BY t.id DESC LIMIT 1`).first(); }
export async function memberColumn(env) { const rc = await cols(env, "registrations"); return rc.includes("member_id") ? "member_id" : "player_id"; }
export function publicGroups(groups) { return (groups || []).map(g => ({ name: g.name, teams: (g.teams || []).map(t => ({ name: t.name, players: (t.players || []).map(p => ({ full_name: p.full_name, gender: p.gender })) })) })); }
