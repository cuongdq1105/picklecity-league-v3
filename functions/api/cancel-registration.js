...
export async function onRequestPost({request,env}){
  try{
    ...
    await env.DB.prepare("UPDATE registrations SET status='CANCELLED',updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(id).run();
    await audit(env, t?.id, "CANCEL_REGISTRATION", `Registration ${id}`);
    return json({ok:true});
