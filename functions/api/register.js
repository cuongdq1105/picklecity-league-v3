import { json, ensureAll, openTournament, memberColumn, audit } from './_utils.js';
...
export async function onRequestPost({request,env}) {
  try {
    ...
    if(!m){
      await env.DB.prepare(`INSERT INTO members (full_name,phone,gender,level_group,level_score) VALUES (?,?,?,'UNRANKED',1000)`)
        .bind(fullName,phone,gender).run();
      m=await env.DB.prepare('SELECT * FROM members WHERE phone=?').bind(phone).first();
    }
    ...
    const ex=await env.DB.prepare(`
      ...
      LIMIT 1
    `).bind(t.id,m.id).first();
    if(ex) return json({ok:false,error:'Số điện thoại này đã đăng ký giải này rồi.'},{status:409});

    await env.DB.prepare(`INSERT INTO registrations (tournament_id,${col},payment_amount,payment_status,status)
      VALUES (?,?,?,'PLAYER_MARKED_PAID','ACTIVE')`).bind(t.id,m.id,t.fee||150000).run();

    await audit(env, t.id, "REGISTER", `${fullName} đăng ký`);
