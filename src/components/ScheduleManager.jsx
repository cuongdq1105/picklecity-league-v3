...
import { CalendarDays, GitBranch, ClipboardCopy } from "lucide-react";
import { makeSchedule, makeKnockout, calcStandings, exportScheduleText } from "../utils/draw";

export default function ScheduleManager({
  groups, config, setConfig,
  schedule, setSchedule,
  knockout, setKnockout,
  setMsg
}) {
  ...
  return <section className="tournamentConfigBox">
    <h3><CalendarDays size={18}/> Lịch thi đấu & nhánh loại trực tiếp</h3>
    <p className="hint">BTC có thể xếp lịch vòng bảng, nhập tỷ số, xem BXH và sinh nhánh Tứ kết.</p>
      ...
      <div className="tablewrap">
        <table>
          ...
          <tbody>{schedule.map((m,i)=><tr key={m.id}>
            ...
            <td className="scoreInputs">
              <input value={m.homeScore} onChange={e=>updateScore(m.id,"homeScore",e.target.value)} placeholder="0"/>
              <span>-</span>
              <input value={m.awayScore} onChange={e=>updateScore(m.id,"awayScore",e.target.value)} placeholder="0"/>
            </td>
          </tr>)}</tbody>
        </table>
      </div>
    ...
    {Object.keys(standings).length > 0 && schedule.length > 0 && <div className="standingsBox">
      ...
      {Object.entries(standings).map(([group, rows])=><div className="standingGroup" key={group}>
        ...
        <div className="tablewrap"><table>
          <thead><tr><th>Hạng</th><th>Đội</th><th>Trận</th><th>Thắng</th><th>Thua</th><th>Điểm+</th><th>Điểm-</th><th>HS</th></tr></thead>
          <tbody>{rows.map(r=><tr key={r.name}>
            <td>{r.rank}</td><td>{r.name}</td><td>{r.played}</td><td>{r.win}</td><td>{r.loss}</td><td>{r.pf}</td><td>{r.pa}</td><td>{r.diff}</td>
          </tr>)}</tbody>
        </table></div>
      </div>)}
    </div>}
