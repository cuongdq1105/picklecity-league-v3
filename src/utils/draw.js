...
export function makeTeams(players, method="balanced") {
  let list=[...players];
  const teams=[], leftover=[];
  if(method==="balanced"){
    list=shuffle(list).sort((a,b)=>strength(b)-strength(a));
    while(list.length>=2){
      const a=list.shift(), b=list.pop();
    ...
    makeRoundRobin(g.teams||[]).forEach((matches,ri)=>{
      matches.forEach((m,mi)=>{
        ...
        all.push({
          ...
          type:"GROUP",
          status:"SCHEDULED",
          group:g.name, round:ri+1, match:mi+1, court,
...
export function scoreSummary(match){
  const games=(match.games||[]).map(parseGame).filter(Boolean);
  ...
  const winner = homeGames>awayGames ? match.home?.name : awayGames>homeGames ? match.away?.name : "";
...
export function calcStandings(groups, schedule){
  ...
  (groups||[]).forEach(g=>{
    byGroup[g.name]=(g.teams||[]).map(t=>({
      group:g.name, team:t, name:t.name, players:teamLabel(t),
      played:0, win:0, loss:0, gameFor:0, gameAgainst:0, pf:0, pa:0, gameDiff:0, diff:0, rank:0
    }));
  });
  const find=(group,teamName)=>(byGroup[group]||[]).find(r=>r.name===teamName);
  (schedule||[]).filter(m=>m.type!=="KO").forEach(m=>{
    const s=scoreSummary(m);
    if(!s.games.length || !s.winner) return;
    const h=find(m.group,m.home?.name), a=find(m.group,m.away?.name);
    if(!h||!a) return;
    h.played++; a.played++;
    h.gameFor += s.homeGames; h.gameAgainst += s.awayGames;
    a.gameFor += s.awayGames; a.gameAgainst += s.homeGames;
    h.pf += s.homePts; h.pa += s.awayPts;
    a.pf += s.awayPts; a.pa += s.homePts;
    if(s.winner===m.home?.name){ h.win++; a.loss++; }
    else { a.win++; h.loss++; }
  });
  Object.values(byGroup).forEach(rows=>{
    rows.forEach(r=>{r.gameDiff=r.gameFor-r.gameAgainst; r.diff=r.pf-r.pa;});
    rows.sort((a,b)=>b.win-a.win || b.gameDiff-a.gameDiff || b.diff-a.diff || b.pf-a.pf || a.players.localeCompare(b.players,"vi"));
...
export function makeKnockout(groups,cfg,standingsByGroup=null){
  ...
  if(standingsByGroup) selected=selectQualified(standingsByGroup,cfg);
    ...
    const fake={};
    (groups||[]).forEach(g=>fake[g.name]=(g.teams||[]).map((team,i)=>({
      group:g.name,rank:i+1,team,players:teamLabel(team),win:0,gameDiff:0,diff:0,pf:0
    })));
  ...
  for(let i=0;i<Math.floor(selected.length/2);i++){
    ...
    if(a&&b) pairs.push({id:`QF-${i+1}`, name:`Tứ kết ${i+1}`, type:"KO", round:"QF", status:"SCHEDULED", a, b, games:[{home:"",away:""}], winner:""});
  }
  return pairs;
}

export function koNextRound(matches,label,round){
  const winners=(matches||[]).filter(m=>m.winner).map(m=>({slot:m.winner, team:{name:m.winner,players:[]}}));
  const pairs=[];
  for(let i=0;i<Math.floor(winners.length/2);i++){
    pairs.push({id:`${round}-${i+1}`,name:`${label} ${i+1}`,type:"KO",round,status:"SCHEDULED",a:winners[i],b:winners[winners.length-1-i],games:[{home:"",away:""}],winner:""});
  }
  return pairs;
}

export function exportScheduleText(schedule){
  return (schedule||[]).map((m,i)=>`${i+1}. ${m.time?m.time+" - ":""}Sân ${m.court||""} - ${m.group||m.round}: ${m.home?.name||m.a?.slot} vs ${m.away?.name||m.b?.slot}`).join("\n");
}
